 ...
 */
categoryRouter.post("/", authentication,authorization(["admin", "dataEntry"]), multerHost("category").fields([{ name: 'image', maxCount: 1 }]) ,validation(csSchema.createCategory),asyncHandler(cs.createCategory))
 ...
 * @apiError {String} message Error message if category not found
 */
categoryRouter.put("/:id" ,authentication,authorization(["admin", "dataEntry"]), multerHost("category").fields([{ name: 'image', maxCount: 1 }]) ,validation(csSchema.updateCategory),asyncHandler(cs.updateCategory))
