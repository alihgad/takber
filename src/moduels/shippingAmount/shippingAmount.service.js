...
export const getShippingAmountsAdmin = async (req, res) => {
    try {
        const shippingAmounts = await ShippingAmount.find({}).sort({ createdAt: -1 });
        
        res.status(200).json({
            message: "All Shipping amounts retrieved successfully",
            shippingAmounts
        });
    } catch (error) {
        res.status(500).json({
...
export const getShippingAmountsUser = async (req, res) => {
    try {
        ...
        res.status(200).json({
            message: "Active Shipping amounts retrieved successfully",
            shippingAmounts
        });
    } catch (error) {
        res.status(500).json({
